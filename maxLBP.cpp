	...
	n2 = 0;
		...
		while(true){
			...
			for(int i=0;i<l;i++){
				...
				int counter = 0;
				int tempIndex = 0;
				double tempValue = exp(tempBelief[0]);
				
				for(int j=1;j<factorSize;j++){
					

					if((j!=0 && j%10==0)){
						tempMsg[counter] = log(tempValue);
						tempValue = exp(tempBelief[j]);
						j++;
						counter++;
					}

					
					double potentialBelief = exp(tempBelief[j]);

					if(potentialBelief > tempValue){

						tempValue = potentialBelief;
				...
				for(int j=0;j<msgSize;j++){
					...
					double tempValue = exp(tempBelief[j]);

					for(int k=j+msgSize;k<factorSize;k=k+msgSize){
						double potentialValue = exp(tempBelief[k]);
						if(tempValue < potentialValue){
							tempValue = potentialValue;
						}
					}
					tempMsg[j] = log(tempValue);
